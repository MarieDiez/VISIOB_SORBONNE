...
def calcGrad_Ori(frame, th):
	...
	return grad, orientation, oriValid, notvalidIndex, validIndex
...
while(1):
	...
	if ret == True:
		...
		tHough = np.zeros_like(orientation)

		tHough = calcHough(tHough, orientation, rTable, validIndex)

		#mean shift
		ret, track_window = cv2.meanShift(tHough, track_window, term_crit)
		...
		if k == 27:
				...
				cv2.imwrite('./images/Q5_Frame_tHough_%04d.png'%cpt,tHough)
