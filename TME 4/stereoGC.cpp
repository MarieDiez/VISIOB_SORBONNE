...
int main() {
    ...
    const int n = 3;         // Consider correlation patches of size (2n+1)*(2n+1) NCC
    ...
    // plus lambda fort => plus smoothness importante et plus meme label
    ...
    // for toys images
    const int dmin = 7;     // Minmum disparity
    const int dmax = 30;     // Maximum disparity
    ...
    //Graph<int,int,int> G(0,0);
    int nb_nodes = nx*ny*nd;
    int nb_edges = nx*ny*nd + (nx-1)*ny*nd + nx*(ny-1)*nd;
    Graph<int, int, int> G(nb_nodes, nb_edges);
    G.add_node(nb_nodes);

    double sum_lambda = 4*lambda;
    double Kp = 1 + (nd-1) * sum_lambda;
    for (int i=0; i < nx; i++){
        int x = i * zoom + n;
        for (int j=0; j < ny; j++){
            int y = n+j*zoom;

            // values to first and last layer
            double Dp_min = zncc(I1, I1M, I2, I2M, x, y, x+dmin, y, n);
            double Dp_max = zncc(I1, I1M, I2, I2M, x, y, x+dmax, y, n);
            int ij_ind = (j*nx + i);
            G.add_tweights(ij_ind, (int)(wcc*rho(Dp_min) + Kp), 0);
            G.add_tweights(ij_ind + nx*ny*(nd - 1), 0, (int)(wcc*rho(Dp_max)+ Kp));

            // all others layers
            for (int l=0; l < nd; l++){
                if (l < nd-1){
                    double Dp = zncc(I1, I1M, I2, I2M, x, y, x+dmin+l, y, n);
                    G.add_edge(ij_ind+(nx*ny)*l, ij_ind+(nx*ny)*(l+1), (int)(wcc*rho(Dp) + Kp), 0);
                }
                // lambda weight on neighboors i+1 et j+1
                if (j+1 < ny){
                    G.add_edge(ij_ind + (nx*ny)*l, ((j+1) * nx + i) + (nx*ny)*l, lambda, lambda);
                }
                if (i+1 <nx){
                    G.add_edge(ij_ind + (nx*ny)*l, (j * nx + (i+1)) + (nx*ny)*l, lambda, lambda);
                }
            }
        }
    }

    cout<<endl<<"***\n***Graph creation : code has to be completed!\n***"<<endl;
    ...
    for (int i=0;i<nx;i++) {
        for (int j=0;j<ny;j++) {
            ...
            // look for disparity in sink (target) set
            D(i,j) = -1;
            for (int l=0; l < nd; l++){
                if (G.what_segment((j*nx+i) + (nx*ny)*l) == Graph<int, int, int>::SINK){
                    D(i,j) = dmin+l;
                    break;
                }
            }
            if (D(i,j) == -1){
                cout << "No disparity " << i << " " << j << endl;
            }
            /////  END CODE TO BE COMPLETED
            /////------------------------------------------------------------
        }
